...
public class AsteroidMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // float H = Input.GetAxis("X360_LeftAnalog_X") * moveRate;
        ...
        if (Input.GetAxis("X360_LeftAnalog_X") != 0)
        {
            H = Input.GetAxis("X360_LeftAnalog_X") * moveRate;
        } else if (Input.GetAxis("Horizontal") != 0)
        {
            H = Input.GetAxis("Horizontal") * moveRate;
        }
        else
        {
            H = 0;
        }
        // print("Taxa de movimento horizontal: " + H);
        transform.Translate(new Vector3(H * Time.deltaTime, 0, 0), Space.World);
